...
%carte SD contenant l'environnement logiciel de cette plate-forme et simplifie
%la création de sauvegardes lors de la mise au point de l'environnement
%logiciel.
...
\begin{figure}
    ...
    %\footnote{Ici fabriqué par l'entreprise Micron.}
\end{figure}
...
Qemu\footnote{La bibliothèque cofi\_rpi (libcofi\_rpi.so) propose une
    ...
    % \url{http://archlinuxarm.org/forum/viewtopic.php?f=31&t=5684}
}.
...
%Notez que les options \og{}-no-reboot\fg{} et \og{}-serial stdio\fg{} sont
%facultatives.  Elles indiquent respectivement de quitter qemu à l'extinction du
%système émulé et de rediriger le port série virtuel du système émulé vers le
%terminal courant du système hôte.
...
Pour pouvoir démarrer l'émulation du Raspberry Pi en mode console, nous
utilisons l'option \og{}-nographic\fg{} de Qemu~:
\begin{verbatim}
$ qemu-system-arm \
    -kernel kernel-qemu \
    -cpu arm1176 \
...
% with the monitor (unless redirected elsewhere explicitly). Therefore, you can
% still use QEMU to debug a Linux kernel with a serial console.  Use C-a h for
% help on switching between the console and monitor.

L'option \og{}console=ttyAMA0\fg{} ajoutée dans \og{}-append\fg{} permet quant
à elle d'afficher sur la console hôte les messages du noyau Linux émulé.

\cred{
Pour accéder à la console de configuration de Qemu, ...
}
\begin{verbatim}
...
\section{Réseaux}\label{sec:network}

% http://wiki.qemu.org/Documentation/Networking
% http://doc.ubuntu-fr.org/qemu
% \url{http://superuser.com/questions/690060/how-to-enable-network-with-a-raspberry-pi-emulated-on-qemu}
% \url{http://en.wikibooks.org/wiki/QEMU/Networking}
% \url{http://stackoverflow.com/questions/23106012/how-to-access-raspberry-pi-qemu-vm-via-network}

Il existe quatre méthodes pour connecter notre RaspberryPi virtuel à un réseau~:

\begin{itemize}
    \item {\em User mode networking (SLIRP)}
    \item {\em Socket redirection}
    \item {\em Tap networking}
    \item {\em VDE (Virtual Distributed Ethernet) networking}
\end{itemize}
...
\cred{une fois que vous avez ... votre image ... vous pouvez l'installer sur un vrai RPi avec la commande d'installation classique : dd if=/... of=... . Cf. https://www.raspberrypi.org/documentation/installation/installing-images/README.md for more infos.}
